...
async def check_user_gates(db: AsyncSession, user_id) -> RiskCheckResult:
    ...
    if active_accounts > MAX_ACTIVE_ACCOUNTS_PER_USER:
        return RiskCheckResult(
            False,
            f"Max active accounts exceeded ({active_accounts}/{MAX_ACTIVE_ACCOUNTS_PER_USER})",
        )

    return RiskCheckResult(True)


async def check_account_gates(
    db: AsyncSession, account_id, max_daily_loss: float = MAX_DAILY_LOSS_USD
) -> RiskCheckResult:
    ...
    if daily_loss_abs >= max_daily_loss:
        return RiskCheckResult(
            False, f"Daily loss limit reached: ${daily_loss_abs:.2f} / ${max_daily_loss:.2f}"
        )
...
async def check_cooldown(
    db: AsyncSession, account_id, cooldown_seconds: int = COOLDOWN_BETWEEN_BASKETS_SECONDS
) -> RiskCheckResult:
...
async def check_basket_age(
    db: AsyncSession, account_id, max_age_hours: int = MAX_BASKET_AGE_HOURS
) -> list[Basket]:
        ...
        if loss_mode == "pct_wallet":
            ...
            if wallet_balance > 0:
                ...
                if loss_direction == "exceeds":
                    ...
                    peak_loss_pct = (
                        (peak_loss_usd / wallet_balance) * 100.0 if wallet_balance > 0 else 0.0
                    )
