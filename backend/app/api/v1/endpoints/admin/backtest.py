...
class BacktestRequest(BaseModel):
    ...
    start_date: str | None = Field(default=None, description="Custom start date ISO string")
    end_date: str | None = Field(default=None, description="Custom end date ISO string")
    ...
    tp_fixed_amount: float = Field(
        default=5.0, ge=0, description="Fixed USD TP target when tp_mode='fixed'"
    )
    ...
    atr_multiplier: float = Field(
        default=0.6, ge=0.1, le=2.0, description="ATR multiplier for grid step sizing"
    )
    step_min_pct: float = Field(
        default=0.004, ge=0.001, le=0.05, description="Minimum step size as fraction (0.004 = 0.4%)"
    )
    step_max_pct: float = Field(
        default=0.025, ge=0.005, le=0.10, description="Maximum step size as fraction (0.025 = 2.5%)"
    )

    # Risk management
    max_basket_age_hours: int = Field(
        default=72, ge=0, le=720, description="Force-close baskets older than this. 0 = disabled."
    )

    # Trend filter
    trend_filter_enabled: bool = Field(
        default=False, description="Enable multi-timeframe trend filter"
    )
    trend_timeframes: list[str] = Field(
        default=["1d", "4h"], description="Timeframes for trend detection"
    )
    trend_mode: str = Field(
        default="majority", description="How to combine trend signals: majority/all/any"
    )
    ...
    # Risk controller
    risk_controller_enabled: bool = Field(
        default=False, description="Enable risk controller for liquidation protection"
    )
    rc_max_so_trigger: int = Field(
        default=5, ge=1, le=20, description="SO count that activates loss check"
    )
    rc_margin_usage_pct: float = Field(
        default=80.0, ge=10, le=99, description="Margin guard threshold %"
    )
    rc_max_basket_loss_pct: float = Field(
        default=10.0, ge=1, le=100, description="Max basket loss as % of wallet"
    )
    rc_max_basket_loss_usd: float = Field(default=50.0, ge=0, description="Max basket loss in USD")
    rc_loss_mode: str = Field(
        default="pct_wallet", description="Loss calculation mode: pct_wallet or fixed_usd"
    )
    rc_loss_direction: str = Field(
        default="exceeds", description="Exit direction: exceeds or recovers_to"
    )
    rc_margin_guard_enabled: bool = Field(
        default=True, description="Whether the margin guard check is active"
    )

    # Optional label for the run
    label: str | None = Field(
        default=None, max_length=200, description="Optional label for this backtest run"
    )
...
async def get_backtest_detail(
    ...
    try:
        ...
        raise HTTPException(status_code=400, detail="Invalid backtest ID") from None

    result = await db.execute(select(BacktestHistory).where(BacktestHistory.id == bt_uuid))
