...
from app.api.deps import get_db, get_current_user, require_workspace_role
...
async def get_workspace(
    ...
    member: WorkspaceMember = Depends(require_workspace_role([WorkspaceRole.OWNER, WorkspaceRole.ADMIN, WorkspaceRole.VIEWER])),
    db: AsyncSession = Depends(get_db)
):
    ...
    result = await db.execute(select(Workspace).where(Workspace.id == workspace_id, Workspace.deleted_at == None))
...
async def update_workspace(
    ...
    member: WorkspaceMember = Depends(require_workspace_role([WorkspaceRole.OWNER, WorkspaceRole.ADMIN])),
    db: AsyncSession = Depends(get_db)
):
    if member.workspace_id != workspace_id:
        raise HTTPException(status_code=403, detail="Workspace ID mismatch")
...
async def add_workspace_member(
    ...
    current_member: WorkspaceMember = Depends(require_workspace_role([WorkspaceRole.OWNER, WorkspaceRole.ADMIN])),
    db: AsyncSession = Depends(get_db)
):
