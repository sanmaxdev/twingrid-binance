...
export default function AccountSettingsModal({ account, isOpen, onOpenChange, onSuccess }: AccountSettingsModalProps) {
  const [isSubmitting, setIsSubmitting] = useState(false)
  const [showGuide, setShowGuide] = useState(false)

  const [settings, setSettings] = useState({
    active_symbols: ["BTCUSDT"] as string[],
    leverage: 10,
  ...
  useEffect(() => {
    if (account.settings?.config) {
      const c = account.settings.config
  ...
  const handleSubmit = async (e: React.FormEvent) => {
    ...
    try {
      ...
      await accountsService.updateAccountSettings(account.id, settings)
  ...
  const toggleSymbol = (sym: string) => {
    setSettings(prev => {
      ...
      if (current.includes(sym)) {
        // Don't allow deselecting the last symbol
        if (current.length <= 1) {
          toast.error("At least 1 trading pair must be selected")
          return prev
        }
        return { ...prev, active_symbols: current.filter(s => s !== sym) }
      } else {
        if (current.length >= MAX_ACTIVE_SYMBOLS) {
          toast.error(`Maximum ${MAX_ACTIVE_SYMBOLS} pairs allowed`)
          return prev
        }
        ...
        <form onSubmit={handleSubmit}>
          <DialogHeader>
            <div className="flex items-center justify-between">
              <DialogTitle>TWIN GRID Strategy Settings</DialogTitle>
              <button type="button" onClick={(e) => { e.preventDefault(); e.stopPropagation(); setShowGuide(true); }}
                className="flex items-center gap-1 text-[11px] font-medium text-[#F0B90B] hover:text-[#D4A20B] transition-colors mr-6">
                <HelpCircle className="h-3.5 w-3.5" /> Settings Guide
              </button>
            </div>
            <DialogDescription>
              Configure trading parameters for {account.name}.
            </DialogDescription>
          </DialogHeader>
          ...
          <div className="grid gap-5 py-4">
            ...
            <div>
              ...
              <div className="grid grid-cols-2 gap-2 mb-3">
                <button
                  type="button"
                  onClick={() => setSettings(prev => ({ ...prev, sizing_mode: "fixed_usd" }))}
                ...
                <button
                  type="button"
                  onClick={() => setSettings(prev => ({ ...prev, sizing_mode: "pct_capital" }))}
                  className={`py-2.5 px-3 rounded-lg border text-sm font-semibold transition-all duration-200 ${
                    settings.sizing_mode === "pct_capital"
                      ? "border-[#F0B90B] bg-[#F0B90B]/10 text-[#F0B90B]"
                      : "border-neutral-700 bg-neutral-900/50 text-neutral-400 hover:border-neutral-500"
                  }`}
                >
              ...
              <div className="grid grid-cols-2 gap-4">
                {settings.sizing_mode === "fixed_usd" ? (
                  <div className="grid gap-1.5">
                    <label htmlFor="base_order_usd" className="text-sm font-medium text-neutral-300">Base Order ($)</label>
                    <Input
                      id="base_order_usd"
                      name="base_order_usd"
                      type="number"
                  ...
                  <div className="grid gap-1.5">
                    <label htmlFor="base_order_pct" className="text-sm font-medium text-neutral-300">Base Order (%)</label>
                    <Input
                      id="base_order_pct"
                      name="base_order_pct"
                      type="number"
                ...
                <div className="grid gap-1.5">
                  <label htmlFor="leverage" className="text-sm font-medium text-neutral-300">Leverage (x)</label>
                  <Input
                    id="leverage"
                    name="leverage"
                    type="number"
            ...
            <div>
              <div className="flex items-center justify-between mb-2">
                ...
                <button
                  type="button"
                  onClick={() => setSettings(prev => ({ ...prev, compounding_enabled: !prev.compounding_enabled }))}
                  className={`relative w-10 h-5 rounded-full transition-colors duration-200 ${
                    settings.compounding_enabled ? "bg-[#0ECB81]" : "bg-neutral-700"
                  }`}
                >
                  <span className={`absolute top-0.5 left-0.5 w-4 h-4 bg-white rounded-full transition-transform duration-200 ${
                    settings.compounding_enabled ? "translate-x-5" : "translate-x-0"
                  }`} />
                </button>
              </div>
              {settings.compounding_enabled && (
                <div className="grid gap-1.5">
                  <label htmlFor="compounding_pct" className="text-sm font-medium text-neutral-300">
                    Growth Scale (%)
                    <span className="text-xs text-neutral-500 ml-1">— 100% = scale proportionally with account growth</span>
                  </label>
                  <Input
                    id="compounding_pct"
                    name="compounding_pct"
                    type="number"
                    step="10"
                    min="10"
                    max="500"
                    value={settings.compounding_pct}
                    onChange={handleChange}
                  />
                </div>
              )}
            ...
            <div>
              ...
              <div className="grid grid-cols-2 gap-4">
                <div className="grid gap-1.5">
                  <label htmlFor="max_safety_orders" className="text-sm font-medium text-neutral-300">Max Safety Orders</label>
                  <Input
                    ...
                    min="0"
                    max="20"
                    value={settings.max_safety_orders}
                    onChange={handleChange}
                    required
                  />
                </div>
                <div className="grid gap-1.5">
                  ...
                  <div className="grid grid-cols-2 gap-2 mb-2">
                    <button
                      type="button"
                      onClick={() => setSettings(prev => ({ ...prev, tp_mode: "pct" as const }))}
                      className={`py-1.5 px-2 rounded-lg border text-xs font-semibold transition-all duration-200 ${
                    ...
                    <button
                      type="button"
                      onClick={() => setSettings(prev => ({ ...prev, tp_mode: "fixed" as const }))}
                      className={`py-1.5 px-2 rounded-lg border text-xs font-semibold transition-all duration-200 ${
                        settings.tp_mode === "fixed"
                          ? "border-[#F0B90B] bg-[#F0B90B]/10 text-[#F0B90B]"
                          : "border-neutral-700 bg-neutral-900/50 text-neutral-400 hover:border-neutral-500"
                      }`}
                    >
                  ...
                  {settings.tp_mode === "pct" ? (
                    ...
                    <Input
                      id="tp_fixed_amount"
                      name="tp_fixed_amount"
                      type="number"
                      step="0.5"
                      min="0.5"
                      value={settings.tp_fixed_amount}
                      onChange={handleChange}
                      required
                    />
                  )}
                  <p className="text-[10px] text-neutral-500 mt-0.5">
                    {settings.tp_mode === "pct"
                      ? "Target profit as a % of your wallet balance per basket."
                      : "Fixed USD profit target per basket."
                    }
                  </p>
                </div>
                <div className="grid gap-1.5">
                  <label htmlFor="volume_scale" className="text-sm font-medium text-neutral-300">Volume Scale</label>
                  <Input
                    id="volume_scale"
                    name="volume_scale"
                    type="number"
                    step="0.1"
                    min="1.0"
                    value={settings.volume_scale}
                    onChange={handleChange}
                    required
                  />
                </div>
                <div className="grid gap-1.5">
                  <label htmlFor="step_scale" className="text-sm font-medium text-neutral-300">Step Scale</label>
                  <Input
                    id="step_scale"
                    name="step_scale"
                    type="number"
            ...
            <div>
              ...
              <div className="grid gap-1.5">
                <label htmlFor="max_basket_age_hours" className="text-sm font-medium text-neutral-300">Max Basket Age</label>
                <select
                  id="max_basket_age_hours"
                ...
                <p className="text-[11px] text-neutral-500">Force-close baskets older than this. Set to "Off" to disable.</p>
              </div>
            </div>
            ...
            <div>
              ...
              <div className="mb-3">
                <div className="flex items-center justify-between mb-1.5">
                  <label htmlFor="signal_threshold" className="text-sm font-medium text-neutral-300">
                    Entry Threshold
                  </label>
                  <span className={`text-sm font-bold font-mono px-2 py-0.5 rounded ${
                    settings.signal_threshold <= 40 ? "text-[#F6465D] bg-[#F6465D]/10" :
                    settings.signal_threshold <= 55 ? "text-[#F0B90B] bg-[#F0B90B]/10" :
              ...
              <div className="grid grid-cols-2 gap-4">
                <div className="grid gap-1.5">
                  <label htmlFor="rsi_long_threshold" className="text-sm font-medium text-neutral-300">RSI Long (&lt;)</label>
                  <Input
                    id="rsi_long_threshold"
                    name="rsi_long_threshold"
                    type="number"
                    min="20"
                    max="50"
                    value={settings.rsi_long_threshold}
                    onChange={handleChange}
                  />
                </div>
                <div className="grid gap-1.5">
                  <label htmlFor="rsi_short_threshold" className="text-sm font-medium text-neutral-300">RSI Short (&gt;)</label>
                  <Input
                    id="rsi_short_threshold"
                    name="rsi_short_threshold"
                    type="number"
                    min="50"
                    max="80"
                    value={settings.rsi_short_threshold}
                    onChange={handleChange}
                  />
                </div>
              </div>
            ...
            <div>
              ...
              <div className="grid grid-cols-2 gap-2">
                <button
                  type="button"
                  onClick={() => setSettings(prev => ({ ...prev, allow_long: !prev.allow_long }))}
                  className={`py-2.5 px-3 rounded-lg border text-sm font-semibold transition-all duration-200 ${
                    settings.allow_long
                ...
                >
                  🟢 Long {settings.allow_long ? "ON" : "OFF"}
                </button>
                <button
                  type="button"
                  onClick={() => setSettings(prev => ({ ...prev, allow_short: !prev.allow_short }))}
                  className={`py-2.5 px-3 rounded-lg border text-sm font-semibold transition-all duration-200 ${
                    settings.allow_short
                      ? "border-[#F6465D]/50 bg-[#F6465D]/10 text-[#F6465D]"
                      : "border-neutral-700 bg-neutral-900/50 text-neutral-500"
                  }`}
                >
                  🔴 Short {settings.allow_short ? "ON" : "OFF"}
                </button>
              </div>
            </div>
            ...
            <div className="border border-[#2B2F36] rounded-xl p-4 bg-[#0B0E11]/30">
              <div className="flex items-center justify-between mb-3">
                <div className="flex items-center gap-2">
                  ...
                  <div>
                    ...
                    <p className="text-[10px] text-neutral-500 mt-0.5">Auto-close losing baskets to prevent liquidation</p>
                  </div>
                </div>
                <button
                  type="button"
                  onClick={() => setSettings(prev => ({ ...prev, risk_controller_enabled: !prev.risk_controller_enabled }))}
              ...
              {settings.risk_controller_enabled && (
                <div className="space-y-3 mt-3 pt-3 border-t border-[#2B2F36]">
                  {/* SO Trigger */}
                  <div className="grid gap-1.5">
                    <label className="text-xs text-neutral-400">SO Trigger (activate after X SOs filled)</label>
                    <Input
                      type="number"
                      min="1"
                      max="20"
                      value={settings.rc_max_so_trigger}
                  ...
                  {/* Loss Mode */}
                  <div className="grid gap-1.5">
                    <label className="text-xs text-neutral-400">Affordable Loss Mode</label>
                    <div className="grid grid-cols-2 gap-1.5">
                      <button type="button" onClick={() => setSettings(prev => ({ ...prev, rc_loss_mode: "pct_wallet" as const }))}
                        className={`py-2 rounded-lg text-xs font-bold transition-all ${settings.rc_loss_mode === "pct_wallet"
                          ...
                          : "bg-neutral-900 border border-neutral-700 text-neutral-500"}`}>% of Wallet</button>
                      <button type="button" onClick={() => setSettings(prev => ({ ...prev, rc_loss_mode: "fixed_usd" as const }))}
                        className={`py-2 rounded-lg text-xs font-bold transition-all ${settings.rc_loss_mode === "fixed_usd"
                          ? "bg-[#F0B90B]/10 border border-[#F0B90B]/30 text-[#F0B90B]"
                          : "bg-neutral-900 border border-neutral-700 text-neutral-500"}`}>Fixed USD</button>
                    </div>
                  </div>
                  ...
                  <div className="grid gap-1.5">
                    <label className="text-xs text-neutral-400">
                      {settings.rc_loss_mode === "pct_wallet" ? "Max Loss (% of wallet)" : "Max Loss (USD)"}
                    </label>
                    <Input
                      type="number"
                      min="1"
                      step={settings.rc_loss_mode === "pct_wallet" ? "1" : "5"}
                  ...
                  <div className="grid gap-1.5">
                    ...
                    <p className="text-[10px] text-neutral-500">
                      {settings.rc_loss_direction === "exceeds"
                        ? "Close basket when loss exceeds the threshold (standard stop-loss)."
                        : "Allow deeper loss — close only when PnL recovers back to the threshold."}
                    </p>
                  </div>
                    ...
                    {settings.rc_margin_guard_enabled && (
                      <>
                        <Input
                          ...
                          max="99"
                          step="5"
                          value={settings.rc_margin_usage_pct}
                          onChange={(e) => setSettings(prev => ({ ...prev, rc_margin_usage_pct: parseFloat(e.target.value) || 80 }))}
                        />
                        <p className="text-[10px] text-neutral-500">Close if margin usage exceeds this % — independent of SO count</p>
                      </>
                    )}
                    {!settings.rc_margin_guard_enabled && (
                      <p className="text-[10px] text-[#F6465D]/70">Margin Guard disabled — no emergency margin protection active</p>
                    )}
                  </div>
                </div>
              )}
